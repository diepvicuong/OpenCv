...
void LightColor::morphOps(Mat &thresh) {
	...
	Mat erodeKernel = getStructuringElement(MORPH_RECT, Size(3, 3)); // create arr
	//dilate with larger element so make sure object is nicely visible
	Mat dilateKernel = getStructuringElement(MORPH_RECT, Size(8, 8));

	erode(thresh, thresh, erodeKernel);
	erode(thresh, thresh, erodeKernel);
...
LightColor::LightColor(LightState state, Mat hsv_image) {
	...
	case YELLOW: {
		...
		inRange(hsv_image, Scalar(20, 100, 100), Scalar(30, 255, 255), lower_yellow_hue_range);
		inRange(hsv_image, Scalar(20, 100, 100), Scalar(30,255,255), upper_yellow_hue_range);
	...
	case GREEN: {
		...
		//inRange(hsv_image, Scalar(221,239,21), Scalar(255,250, 220), lower_green_hue_range);
